 ...
 *
 * Description: ¼ÓÓÍ»úĞ­Òé×ª»»Ä£¿é -- ±¦µÂ¶û  4 Line CL, 4800,8,e,1
 *
...
//			ret = DoMode();	      // Unsupported
		...
		case 0x4D:
			...
			/*
			 * Ì§Ç¹Ö®ºóÏÂ·¢Ô¤ÖÃËùÊ¹ÓÃµÄ±ê¼Ç±äÁ¿ºÍ»º³åÇø:
			 * pump.addInf[pos][0]: Ô¤ÖÃ±ê¼Ç, ±êÊ¶ÊÇ·ñÌ§Ç¹ºóÊÇ·ñÓĞÔ¤ÖÃĞèÒªÏÂ·¢
			 * pump.addInf[pos][1]: Ô¤ÖÃ·½Ê½±ê¼Ç×Ö½Ú
			 * pump.addInf[pos][2]: Ô¤ÖÃÔÊĞíÓÍÇ¹, 0xFF - ÔÊĞíÈ«²¿
			 * pump.addInf[pos][3-5]:  Ô¤ÖÃÁ¿»º´æ
			 */
			pump.addInf[pos][0] = 1;            // Ô¤ÖÃ±ê¼Ç, ±ê¼ÇÓĞÔ¤ÖÃĞèÒªÏÂ·¢
			pump.addInf[pos][1] = pump.cmd[0];  // Ô¤ÖÃ·½Ê½
...
static int DoRation(int pos)  
{
	...
	if (pump.addInf[pos][1] == 0x3D) {    // kind = 0xB9 ¶¨Á¿; kind = 0xB5 ¶¨¶î
		kind = 0xB9;
	} else {
		kind = 0xB5;
	}
...
//				gpIfsf_shm->node_online[IFSFpos] = node;
		...
		case PUMP_CALL:					// Ì§Ç¹
			/*
			 * Note: Ô¤ÖÃ¼ÓÓÍ¼Óµ½»òÕß¼ÓÓÍÖĞÖ´ĞĞÍ£Ö¹¼ÓÓÍ, ×´Ì¬¾ù»á±äÎªPUMP_CALL
			 */
			if ((dostop_flag[i] != 0)) {
