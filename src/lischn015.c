 ...
 *
 * Description: 加油机协议转换模块 - 佳力佳, CL_JLJ, 9600,E,8,1 
 *
 * History:
 *
 */
			...
			for (j = 0; j < pump.panelCnt; j++) {
				...
				for (idx = 0; idx < gpIfsf_shm->gunCnt; idx++) {
					if ((gpIfsf_shm->gun_config[idx].node == node) &&
						...
						/*
						 * 佳力佳枪号(面板号)默认0/1, 固定不变
						 * 为统一多枪机地址配置, 配置统一为 1/2
						 * 所以这里需要减一
						 */
				...
				pump.addInf[j][0] = 0;   // 表示是否做了预置
				pump.addInf[j][1] = 0;   // 表示预置是否不允许, 是否需要改变状态为Idle
			}
...
//			ret = DoMode();	      // Unsupported
		...
		case 0x4D:
			...
			if (ret == 0) {
				...
				pump.addInf[pos][0] = 1;  // 设置预置, 需抬枪后才能下发授权
			} else {
				SendCmdAck(NACK, msgbuffer);
				ChangeFP_State(node, pos + 0x21,
					lpDispenser_data->fp_data[pos].FP_State, NULL);
...
//				gpIfsf_shm->node_online[IFSFpos] = node;
		...
		case FP_STOP:                                  // 停止加油
			// 情况1. 加油停止 / 加油到预置值
#if 0
			// 此项目不需要模拟 SuSpended_Fuelling 状态
			...
			if ((__u8)FP_State_Authorised == lpDispenser_data->fp_data[i].FP_State) {
				...
				pump.addInf[i][0] = 0;
				pump.addInf[i][1] = 1;   // 挂枪后再ChangeFP_State to Idle
//				ret = ChangeFP_State(node, i + 0x21, FP_State_Idle, NULL);
			}
		...
		case FP_CALLING:                               // 抬枪
			...
			if ((__u8)FP_State_Calling > lpDispenser_data->fp_data[i].FP_State) {
				memcpy(&predata[i][0], &pump.cmd[pump.cmdlen - 5], 4);  // DoAuth需要
				ret = DoCall(i);
				fp_state[i] = 1;
			...
			if (pump.addInf[i][0] != 0) {
				...
				ret = DoAuth(i);
				if (ret < 0) {
					RunLog("Node[%d]FP[%d]预置加油授权失败, addInf:%02x", node, i + 1,
							pump.addInf[i][0]);
				}
				lpDispenser_data->fp_data[i].Log_Noz_State = 0x01;
			...
			for (i = 0; i < pump.panelCnt; i++) {
				...
				CriticalLog("下发变价命令给节点[%02d]面[%d]枪[%d]......", node, i + 1, j + 1);
					...
					for (trycnt = 0; trycnt < TRY_TIME; trycnt++) {
						...
						if (pump.cmd[4] != 0x00) {
							if (pump.cmd[4] == 0x01) {
								RunLog("Node[%d].FP[%d]当前状态不允许变价", node, i + 1);
							}
							RunLog("Node[%d].FP[%d]变价失败", node, i + 1);
...
static int ClsAllTR(int pos)
{
	...
	do {
		...
		if (ret < 0) {
			RunLog("发上传清除明细命令到Node[%d].FP[%d]失败", node, pos + 1);
			continue;
		}
		...
		if (ret < 0) {
			RunLog("发上传清除明细命令到Node[%d].FP[%d]成功, 取返回信息失败", node, pos + 1);
			continue;
		} 
...
RunLog("predata[%d][0] = %s", pos, Asc2Hex(&predata[pos][0], 4));
		...
		ret = WriteToPort(pump.fd, pump.chnLog, pump.chnPhy, pump.cmd,
			       	pump.cmdlen, 1, pump.wantLen, TIME_OUT_WRITE_PORT);
		if (ret < 0) {
			RunLog("发取授权命令到Node[%d].FP[%d]失败", node, pos + 1);
			continue;
		}
		...
		ret = GetReturnMsg(GUN_MSG_TRAN, pump.chnLog, pump.cmd, &pump.cmdlen);
		if (ret < 0) {
			RunLog("发取授权命令到Node[%d].FP[%d]成功, 取返回信息失败", node, pos + 1);
			continue;
		} 
		...
		if (pump.cmd[5] != 0x01) {  // 失败重试
			continue;
		}

		memcpy(&predata[pos][0], default_predata, 4);   // 默认加满
		return 0;
	}
...
static int DoBusy(int pos)
{
	...
	for (trycnt  = 0; trycnt < 2; ++trycnt) {
		...
		ret = WriteToPort(pump.fd, pump.chnLog, pump.chnPhy, pump.cmd,
			       	pump.cmdlen, 1, pump.wantLen, TIME_OUT_WRITE_PORT);
		if (ret < 0) {
			RunLog("发取实时数据命令到Node[%d].FP[%d]失败", node, pos + 1);
			continue;
		}
		...
		ret = GetReturnMsg(GUN_MSG_TRAN, pump.chnLog, pump.cmd, &pump.cmdlen);
		if (ret < 0) {
			RunLog("发取实时数据命令到Node[%d].FP[%d]成功, 取返回信息失败", node, pos + 1);
			continue;
		} 
...
static int GetPumpOnline()
{
	...
	for (trycnt  = 0; trycnt < TRY_TIME; ++trycnt) {
		...
		ret = GetReturnMsg(GUN_MSG_TRAN, pump.chnLog, pump.cmd, &pump.cmdlen);
		if (ret < 0) {
			RunLog("发通讯测试命令到Node[%d]成功, 取返回信息失败", node);
			continue;
		} 

		if ((pump.cmd[3] ^ pump.cmd[4]) != pump.cmd[5]) {  // 因长度固定, 所以直接指定, 尽可能节省资源
			RunLog("Node[%d]返回数据校验错误", node);
			continue;
		}
