 ...
 *
 * Description: 加油机协议转换模块 -- 恒山型号TQC UDC, 3 Line CL, 9600,8,n,1
 *
...
static PUMP pump;
static int FpGunCnt[MAX_FP_CNT] = {0}; //记录每个面配置的实际油枪数
static int FpGunNo[MAX_FP_CNT][MAX_FP_LN_CNT] = {0};
static int node;
static int IFSFpos;
...
static unsigned char state_have_upto4[MAX_CHANNEL_CNT][MAX_FP_CNT];//判断油机是否大于过4状态
...
int ListenChn020(int chnLog)
{
	...
	RunLog("Listen Channel  020 ,logic channel[%d]", chnLog);
	...
	for (i = 0; i < MAX_CHANNEL_CNT; i++) {
		if (chnLog == gpIfsf_shm->node_channel[i].chnLog) {
			...
			for (j = 0; j < pump.panelCnt; j++) {
				...
				//记录每个面配置的实际油枪数
	                        FpGunCnt[j] = gpIfsf_shm->node_channel[i].cntOfFpNoz[j];
...
//                      RunLog("idx: %d, disabled , skip ......", j);
                ...
                if(gpIfsf_shm->gun_config[j].node == node){
                        if(tmp_fp != gpIfsf_shm->gun_config[j].logfp -1){ //另一面的循环
                                i = 0; 
                        }    
     
                        tmp_fp = gpIfsf_shm->gun_config[j].logfp -1;
                        FpGunNo[ gpIfsf_shm->gun_config[j].logfp -1 ][i++] = gpIfsf_shm->gun_config[j].logical_gun_no;
//                      RunLog("#####node %d fp %d FpGunNo[%d][%d] = %d", node, gpIfsf_shm->gun_config[j].logfp
//                              ,gpIfsf_shm->gun_config[j].logfp -1, i -1, FpGunNo[ gpIfsf_shm->gun_config[j].logfp -1 ][i-1]);
                }    
        }    
...
//			ret = DoMode();	      // Unsupported
		...
		case 0x6D:
                        ...
                        if(ret == 0){
                                ...
                                cmd[1] = 0x55;
                                cmd_lg = sizeof(cmd);
                                ret = SendMsg(TOTAL_MSG_TRAN, cmd , cmd_lg, pump.indtyp, 3);
//                              RunLog("######Node[%d]读取泵码成功!", node);                                      
                        }else{
                                cmd[0] = 0xaa;
                                cmd[1] = 0xee;
                                cmd_lg = sizeof(cmd);
                                ret = SendMsg(TOTAL_MSG_TRAN, cmd , cmd_lg, pump.indtyp, 3);
//                              RunLog("######Node[%d]读取泵码失败!", node);
                        }
                        break;
		case 0x2F:
		...
		case FP_AUTHORISED:
			if ((__u8)FP_State_Calling == lpDispenser_data->fp_data[i].FP_State) {
				ret = ChangeFP_State(node, i + 0x21, FP_State_Started, NULL);
				 if (ret == 0){
                                   state_have_upto4[node][i] = 1;
                                   RunLog("state_have_upto4[%d][%d] = %d", node, i, state_have_upto4[node][i]);
                            }
...
static int DoEot(int pos)
{
	...
	/* 解决恒山新款UDC挂枪后泵码延迟变化,如果发现显示有变化多次取泵码*/
	err_cnt= 0;
	do {
	...
	/*保存此次加油量*/
	display_volume[0] = 0x00;
	display_volume[1] = 0x00;
	display_volume[2] = pump.cmd[14];
	display_volume[3] = pump.cmd[12];
	display_volume[4] = pump.cmd[10];
	RunLog("Node[%d].FP[%d] 屏显加油量:%02x%02x%02x%02x.%02x",node, pos + 1,
		display_volume[0], display_volume[1], display_volume[2], display_volume[3], display_volume[4]);
#if 0
	/*获取上次结束泵码*/
	pr_no = (lpDispenser_data->fp_ln_data[pos][pump.gunLog[pos] - 1]).PR_Id - 1;  // 0 - 7	
	if ((pr_no < 0) || (pr_no > 7)) {
	...
	bzero(vol_total, NOZ_TOTAL_LEN);
	bzero(amount_total, NOZ_TOTAL_LEN);
	
	//恒山4面UDC油机读取泵码延迟较大，此版本不读取泵码

	if( (display_volume[2] | display_volume[3] |display_volume[4]) != 0){ //如果屏显不为零
		for( i = 0 ; i < 8; i++){
			err_cnt = 0;
			do {
			...
			ret = BBcdMinus(&vol_total[1], 4, &pre_total[3], 4, tmp_total);
			if(memcmp(tmp_total, display_volume, sizeof(display_volume)) == 0){
				RunLog("Node[%d].FP[%d]  STotal: %02x%02x%02x%02x.%02x  -  ETotal: %02x%02x%02x%02x.%02x = \
%02x%02x%02x%02x.%02x  泵码差等于屏显加油量",
					node, pos + 1, 
					pre_total[2], pre_total[3], pre_total[4], pre_total[5], pre_total[6],
					vol_total[0], vol_total[1], vol_total[2], vol_total[3], vol_total[4],
					tmp_total[0], tmp_total[1], tmp_total[2], tmp_total[3], tmp_total[4]);
				break;
			}else{
				RunLog("Node[%d].FP[%d]  STotal: %02x%02x%02x%02x.%02x  -  ETotal: %02x%02x%02x%02x.%02x = \
%02x%02x%02x%02x.%02x  泵码差小于屏显加油量",
					node, pos + 1, 
					pre_total[2], pre_total[3], pre_total[4], pre_total[5], pre_total[6],
					vol_total[0], vol_total[1], vol_total[2], vol_total[3], vol_total[4],
					tmp_total[0], tmp_total[1], tmp_total[2], tmp_total[3], tmp_total[4]);
				Ssleep(50000 * i + 1); /*增加延时获取泵码*/
				continue;
			}
		}
	}else{
	...
	RunLog("gpIfsf_shm->state_have_upto4[%d][%d] = %d", node, pos,  state_have_upto4[node][pos]);
		
	if ((display_volume[2] | display_volume[3] |display_volume[4]) != 0 && state_have_upto4[node][pos] ) {

		RunLog("gpIfsf_shm->state_have_upto4[%d][%d] = %d", node, pos,  state_have_upto4[node][pos]);
		state_have_upto4[node][pos] = 0;
		gpIfsf_shm->get_total_flag = 0;
		err_cnt= 0;
		do {
			ret = GetDisplayData(pos);	// 取最后一笔加油记录
			if (ret != 0) {
		...
		err_cnt = 0;
		do {
			ret = EndTr4Fp(node, pos + 0x21, total, 
...
static int InitPumpTotal()
{
	...
	for (i = 0; i < pump.panelCnt; i++) {
		for (j = 0; j < pump.gunCnt[i]; ++j) {
			if((FpGunNo[i][0] == j + 1) || (FpGunNo[i][1] == j + 1) 
				||(FpGunNo[i][2] == j + 1) ||(FpGunNo[i][3] == j + 1) ){  //只读取已配置油枪泵码
				...
				bzero(vol_total, sizeof(vol_total));
				bzero(amt_total, sizeof(amt_total));
			}
