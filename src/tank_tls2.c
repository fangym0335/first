/* 				Veederroot - Tank monitor system
	...
	Version:		350R
	This is IFSF protocol program, 不使用VR提供的库函数
	2007.12
*/
...
#define VEED_TC_VOL_USED 		1 // 在同盛项目，上海急水港站使用维德路特不带校罐时，后台需要液位仪的TC Volume(V20)
static int serial_fd=0;
static unsigned char flag[31];

static unsigned char pre_message[MAX_PAR_NUM][MAX_TANK_NUM][8];
static unsigned char pre_point_volum[MAX_TANK_NUM][161];
...
//			__debug(2, ">>> send cmd  reading ");
			 ...
			 if(ret > 0){
				...
				if (gpIfsf_shm->node_online[MAX_CHANNEL_CNT] != 0) {
					RunLog("read message fail, ret: %d", ret);
				}
			}
		} else {
			if (gpIfsf_shm->node_online[MAX_CHANNEL_CNT] != 0) {
...
{	char tmpbuf2[256];
	...
	if (flag[tpno] == 0 && gpIfsf_shm->auth_flag == DEFAULT_AUTH_MODE) {
		if (first_flag == 0) {
		//	sleep(12);  // for test
			RunLog("%s:>>> after sleep 12s, first_flag ......");
		}
...
//				break;
			case 4:			//fuel level
				...
				if (gpGunShm->data_acquisition_4_TCC_adjust_flag != 0) {
					...
					TCCADLog("level_changed_flag: %d, level2: %ld, tr_fin_flag:%d, tmp:%d",
						level_changed_flag, level2, gpGunShm->tr_fin_flag, gpGunShm->tr_fin_flag_tmp);
				}
				break;
			case 5:			//water level
	...
	if (level_changed_flag != 0 || gpGunShm->tr_fin_flag_tmp != 0) {
		...
		TCCADLog("TCC-AD: %d #罐 写液位记录, %02x%02x%01x.%01x%02x (tr_fin_flag:%d, tmp:%d)",
					tpno + 1, gpIfsf_shm->tlg_data.tp_dat[tpno].Product_Level[0],
					gpIfsf_shm->tlg_data.tp_dat[tpno].Product_Level[1],
					(gpIfsf_shm->tlg_data.tp_dat[tpno].Product_Level[2] >> 4),
					(gpIfsf_shm->tlg_data.tp_dat[tpno].Product_Level[2] & 0x0F),
					gpIfsf_shm->tlg_data.tp_dat[tpno].Product_Level[3],
					gpGunShm->tr_fin_flag,
					gpGunShm->tr_fin_flag_tmp);
...
static unsigned int MyCheckSum(char *Source, int Length, char *sReturn, int *CheckSumLen)
{
	...
	unsigned int iResult = 0;
      // RunLog("source [%d %c %c\n", strlen(Source), Source[Length-1] , Source[Length-2]);
...
static int  get_msglen(const unsigned char *message)
{
	...
	if(message == NULL)
		return -1;
	for ( i = 0; (!(message[i] == '&' && message[i+1] == '&')) && i < 4056; i++);
	if ( message[i + 6] != 0x03 || i == 4056){
...
static int make_data(unsigned char *tank_command, unsigned char *data, int type){
	...
	RunLog("tmp_buffer is %s", tmp_buffer);
...
static int read_config()
{
	...
	while(1)
	{
		...
		ret = send_cmd("\x01i60800", buffReturn, 7, 2);
		...
		pthread_mutex_lock(&mutex);
		ret = send_cmd("\x01i60900", buffReturn, 7, 2);
		pthread_mutex_unlock(&mutex);
		bzero(sReturn, sizeof(sReturn));
...
/*		p = "\x01i609001211261011013A54562E023AA52696033AA52696043AA52696&&F3CF\x03";
		...
		/*buffreturn :<SOH>i50ETTYYMMDDHHmmFFFFFFFF&&CCCC<ETX>*/
		/*p = "\x01i50E00121126101141A00000&&FABA\x03";
		 memcpy(buffReturn, p, get_msglen(p)+7);*/
		/*this parameter is different from others ,for there is only one temp for all tanks*/
		
		pthread_mutex_lock(&mutex);
		send_cmd("\x01i50E00", buffReturn, 7, 2);	
		pthread_mutex_unlock(&mutex);
		/*RunLog("buffer 2 %c buffer 3%c buffer 4%c", buffReturn[2], buffReturn[3], buffReturn[4]);
		if (buffReturn[2] != '5' || buffReturn[3] != '0' || buffReturn[4] != 'E'){
			RunLog("buffer 2 %c buffer 3%c buffer 4%c", buffReturn[2], buffReturn[3], buffReturn[4]);
			sleep(10);
			continue;
		}*/
	       RunLog("tmperature compensation is %s", buffReturn);
		...
		pthread_mutex_lock(&mutex);
		ret = send_cmd("\x01i60700", buffReturn, 7, 2);
		pthread_mutex_unlock(&mutex);
		...
		/*buffreturn :<SOH>i50ETTYYMMDDHHmmTTFFFFFFFF
											TTFFFFFFFF&&CCCC<ETX>*/
#if 0
		p = "\x01i6070012112610110145195800024514000003451460000445190800&&F476\x03";
  		memcpy(buffReturn, p, get_msglen(p)+7);
#endif
			else if (ret == 0){
	       		//RunLog("tank diameter %s", buffReturn);
		...
		pthread_mutex_lock(&mutex);
		send_cmd("\x01i60600", buffReturn, 7, 2);
		pthread_mutex_unlock(&mutex);
			...
			if (send_time ==3) {
				...
				if (ret < 0) {
					...
					if (0x00 == buffReturn[1]){
						...
						time_interval = buffReturn[1]&0x0f + (buffReturn[1]&0xf0)*10;
						RunLog("发送罐配置信息成功，返回正常时间间隔为%d %02x%02x", time_interval, buffReturn[0], buffReturn[1]);
					}
				}
			}
			if (sockfd > 0)
...
void tank_tls2(int p)
{	
	int i,newsock, msgid,ret;
	int len,msglen, buf_len, lenstr, num;
	int tpno;
	int alarm_read_cnt = 5;         // 读5次液位数据读一次警报信息
	int read_fail_cnt = 3;          // 连续3次读数据失败才判断为离线
...
*/		
    ...
    ret = tank_init_config(ipname, stano_bcd, &globle_config, &tank_cnt);
	if (ret == 0){
	...
	while(1){
		...
		*/
		...
		// CSSI.校罐项目数据采集处理
		TCCADLog("TCC-AD: tr_fin_flag: %d, tmp: %d", gpGunShm->tr_fin_flag, gpGunShm->tr_fin_flag_tmp);
		...
		pthread_mutex_lock(&mutex);
		ret = send_cmd("\x01i20100", buf, 7, 3);
		pthread_mutex_unlock(&mutex);
