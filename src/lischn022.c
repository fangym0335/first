/*
 * Filename: lischn022.c
 *
 * Description: 加油机协议转换模块—北京长吉,2Line CL / RS485, 5760,E,8,1 
 *
 * History:
 *
 ...
 *     若干其他细节的修改(可对照bak/lischn022.c查看)
 *
...
static int SetPort022();
static int ReDoSetPort022();
...
int ListenChn022(int chnLog)
{
	...
	RunLog("Listen Channel  022 ,logic channel[%d]", chnLog);
...
//	gpIfsf_shm->node_online[IFSFpos] = node;
	...
	while (1) {
		...
		if (gpIfsf_shm->auth_flag - FCC_SWITCH_AUTH != 0) {
			 ...
			 */
			ret = RecvMsg(ORD_MSG_TRAN, pump.cmd, &pump.cmdlen, &indtyp, 0); 
			if (ret == 0) {
				ret = DoCmd();
				if (ret != 0) {
...
static int DoEot(int pos)
{
	...
	unsigned char *debug_amo = (lpDispenser_data->fp_ln_data[pos][pump.gunLog[pos] -1]).Log_Noz_Amount_Total;
	unsigned char *debug_vol = (lpDispenser_data->fp_ln_data[pos][pump.gunLog[pos] -1]).Log_Noz_Vol_Total;
	unsigned char *debug_pri = lpDispenser_data->fp_data[pos].Current_Unit_Price;
	unsigned char *debug_pno = lpDispenser_data->fp_data[pos].Current_Prod_Nb;
	unsigned char *debug_cam = lpDispenser_data->fp_data[pos].Current_Volume;
	unsigned char *debug_cvo = lpDispenser_data->fp_data[pos].Current_Amount;
	...
	if (memcmp(total, pre_total, sizeof(total)) != 0) {
		...
		price[0] = 0x04;
		price[1] = 0x00;
		price[2] = (pump.cmd[15] << 4) | (pump.cmd[14] & 0x0f);
		price[3] = (pump.cmd[13] << 4) | (pump.cmd[12] & 0x0f);
        ...
        RunLog("节点[%02d]面板[%d]枪[%d] ## Amount: %02x%02x%02x%02x%02x Volume:%02x%02x%02x%02x%02x \
				Amount_Total: %02x%02x%02x%02x%02x%02x%02x Volume_Total: %02x%02x%02x%02x%02x%02x%02x \
				Price: %02x%02x%02x%02x Pr_no: %02x%02x%02x%02x", \
				node, pos + 1, pump.gunLog[pos], \
				debug_cam[0],debug_cam[1],debug_cam[2],debug_cam[3],debug_cam[4], \
				debug_cvo[0],debug_cvo[1],debug_cvo[2],debug_cvo[3],debug_cvo[4], \
				debug_amo[0],debug_amo[1],debug_amo[2],debug_amo[3],debug_amo[4],debug_amo[5],debug_amo[6], \
				debug_vol[0],debug_vol[1],debug_vol[2],debug_vol[3],debug_vol[4],debug_vol[5],debug_vol[6], \
				debug_pri[0],debug_pri[1],debug_pri[2],debug_pri[3], \
				debug_pno[0],debug_pno[1],debug_pno[2],debug_pno[3] );
...
static int GetTransData(int pos)
{
	...
	for (trynum = 0; trynum < TRY_TIME; ++trynum) { 
		...
		RunLog("CHN[%d]FP[%d]-->Pump : %02x%02x", node, pos+1, pump.cmd[0], pump.cmd[1]);
		...
		RunLog("CHN[%d]FP[%d]<--Pump : %s(%d)", Asc2Hex(pump.cmd, pump.cmdlen), pump.cmdlen);
...
static int DoBusy(int pos)
{
	...
	for (trynum = 0; trynum < 2; trynum++) {
		...
		ifsf_amount[0] = 0x06;
		ifsf_amount[1] = 0x00;
		ifsf_amount[2] = tmp1[0];
		ifsf_amount[3] = tmp1[1];
		ifsf_amount[4] = tmp1[2];
		ifsf_volume[0] = 0x06;
		ifsf_volume[1] = 0x00;
